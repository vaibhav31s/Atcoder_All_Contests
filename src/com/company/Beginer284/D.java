package com.company.Beginer284;
...
public class D {
    public static void main(String[] args) {
        ...
        int width= 3968;
        int height = 3272;
        BigInteger volume = BigInteger.valueOf(length);
        volume = volume.multiply(BigInteger.valueOf(width));

        volume = volume.multiply(BigInteger.valueOf(height));
        BigInteger a = new BigInteger("1000000000");
        if(volume.compareTo(a) >=0){
            out.println(true);
    ...
    private static boolean issCycle(HashMap<String, ArrayList<String>> graph) {
            HashSet<String> vis = new HashSet<>();

            for(String k :graph.keySet()){
                if(!vis.contains(k)) {

                    if (isCycle(k, graph, vis,"-1")) {
    ...
    return false;
        ...
        private static boolean isCycle(String cur, HashMap<String, ArrayList<String>> graph, HashSet<String> vis, String parent) {
            vis.add(cur);

            for(String k : graph.get(cur)){
               if(!vis.contains(k)){
    ...
    public static void primeFactors(BigInteger n, List<BigInteger> answer)
    {
        ...
        for (; i.compareTo(sq) == 0 || i.compareTo(sq) < 0; i =  i.add(BigInteger.valueOf(2)))
        {
