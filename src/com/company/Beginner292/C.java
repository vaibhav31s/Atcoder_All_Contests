package com.company.Beginner292;
...
//        while (t-- > 0) {
        long n = sc.nextInt();
            ...
            for (long a = 1; a < n; a++) {
                for (long b = a; b < n; b++) {
                    long k = n-  a *b;
                   first.put(k,first.getOrDefault(k,0L)+1);
                }
            }
            long answer =0;
        for (long c = 1; c < n; c++) {
            for (long d = c; d <n; d++) {
                if(first.containsKey(c*d)) answer+= first.get(c*d);
            }
        }
        out.println(answer);
//       int a =  go(n);
...
//            out.print(set.size() + ",");
//        }
//        int[] arr = new int[]{0,1,4,8,14,20,28,37,44,58,64,80,86,108,108,136,134,169,160,198,};
