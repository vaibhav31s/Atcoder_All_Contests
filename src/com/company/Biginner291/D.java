package com.company.Biginner291;
...
public class D {
    ...
    public static void main(String[] args) {
        ...
        backtrack(set, n, pair, 0 );
        out.println(answer);
    }

    private static void backtrack(HashSet<Long> set, int n, int[][] pair, int i) {
        //choose
        if(i == pair.length -1){
            if(set.size() == pair.length) {
                answer++;
                out.println(set);
            }
            return;
        }
        set.add((long) pair[i][0]);
        backtrack(set, n, pair, i+1);
        set.remove((long)pair[i][0]);
        set.add((long) pair[i][1]);
        backtrack(set, n, pair, i+1);
        set.remove((long)pair[i][1]);
