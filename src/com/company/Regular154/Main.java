package com.company.Regular154;
...
Main {
    ...
    public static Long ansewr= Long.MAX_VALUE;
...
//            int[] arr = readarr(sc, n);
    ...
    private static void dfs(char[] first, char[] second, int i) {
        if(i >= first.length) return;


        char ftemp = first[i];
        char stemp = second[i];
        dfs(first, second, i + 1);
        Long f = Long.parseLong(String.valueOf(first));
        Long s = Long.parseLong(String.valueOf(second));
        ansewr = Math.min((f*s)% 998244353,  ansewr);
        swap(first, second, i);
         f = Long.parseLong(String.valueOf(first));
         s = Long.parseLong(String.valueOf(second));
        ansewr = Math.min((f*s)% 998244353,  ansewr);
        dfs(first, second, i + 1);
        first[i] = ftemp;
        second[i] = stemp;
    ...
    private static void swap(char[] first, char[] second, int i ){
        char temp = first[i];
        first[i] =  second[i];
        second[i] = temp;
    }
