...
//        for(int i = 1; i <= n; i++) map.put(i, new TreeMap<>());
        outer:
        while (t-- > 0) {
            int no = sc.nextInt();
            if(no == 1){
                int a = sc.nextInt();
                int b = sc.nextInt();
                map.putIfAbsent(b, new TreeMap<>());
                TreeMap<Integer,Integer> temp = map.get(b);
               temp.put(a, temp.getOrDefault(a,0)+1);
               list.putIfAbsent(a, new TreeSet<>());
               list.get(a).add(b);
                ...
                int a = sc.nextInt();
                TreeMap<Integer,Integer> temp = map.get(a);
                if(temp == null) continue ;
                for(int x :  temp.keySet()){
                    int count = temp.get(x);
                    for(int i = 0; i < count; i++) out.print(x + " ");
                }
                out.println();
            }else{
                int a =sc.nextInt();
                TreeSet<Integer> ke = list.get(a);
                if(ke == null) continue ;
                for(int x : ke){
