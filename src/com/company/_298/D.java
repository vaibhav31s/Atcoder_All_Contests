...
//        sb.deleteCharAt(0);
//        out.println(sb);
        BigInteger mods = new BigInteger("998244353");
        ...
        while(q-- > 0){
            ...
            if(a == 1){
                ...
                Long hash = get_hash(sb);
                hash = get_hash(new StringBuilder(Long.toString(hash)));
                if(answer.containsKey(hash)){
                    out.println(answer.get(hash));
                    continue;
                }
                int as  =  mod(sb, 998244353);
                answer.put(hash, as);
                out.println(as);
            }
        }
    ...
    static int mod(StringBuilder num, int a)
    {
    ...
    public static  long get_hash(StringBuilder s){
            long  h=0;
            for(int i = 0; i < s.length();i++){
                h = ((h*151) + (s.charAt(i)-'a'+1))% (int)(998244353);
            }
            return h;
        }
