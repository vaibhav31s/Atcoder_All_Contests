...
//        int t = sc.nextInt();
        int[] c = readarr(sc,n);
        int[] r = readarr(sc,n);
        PriorityQueue<KeyValue<Integer, Integer>> pq = new PriorityQueue<>((a,b)-> b.key - a.key);
        for(int i = 0; i < n; i++){
            int ci = c[i];
            int ri = r[i];
            if(ci == t){
                pq.add(new KeyValue<>(ri, i+1));
            }
...
//            assert pq.peek() != null;
            if(!pq.isEmpty())
            out.println(pq.peek().value);
            else out.println(1);
        }
        else{
            out.println(pq.poll().value);
        }
