...
public class D {static FastScanner sc = new FastScanner();
    ...
    private static void go(int start, int end, HashMap<Integer, Integer> map) {
        if(start > end) return;
        int mid  = start + (end -start)/2;

        out.println("? " + mid);out.flush();
        int a = sc.nextInt();
        map.put(mid, a);
        if(map.containsKey(mid+1) && map.get(mid+1)  != a){

            out.println("! " + (mid));
            out.flush();
            exit(1);
        }else if(map.containsKey(mid-1) &&  map.get(mid-1)  != a){
            out.println("! " + (mid-1));out.flush();
            exit(1);
        }
