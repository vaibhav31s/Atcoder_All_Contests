...
public class C {
    public static void main(String[] args) {
        ...
        while (t-- > 0) {
            int n =sc.nextInt();
            String Tdash = sc.next();
            String[] arr = readarr(sc, n);
            ...
            for(int ii  : answer) out.print(ii + " ");
        }
    }

    private static boolean check(String s, String tdash) {

        //First condition
        if(s.equals(tdash)) return true;


        int add = 0;
        int  i = 0;
        int j= 0;
        while((s.length() == (tdash.length()  + 1 )) &&   i < s.length() && j < tdash.length()) {
            if(s.charAt(i) == tdash.charAt(j)) {
                i++;
                j++;
            }else{
                i++;
        ...
        if((s.length()  == (tdash.length() +1)) && add <= 1) return true;


        //third
        int deleted = 0;
        i = 0; j= 0;
        while(((s.length() + 1 )== tdash.length()  ) &&   i < s.length() && j < tdash.length()) {
            if(s.charAt(i) == tdash.charAt(j)) {
                i++;
                j++;
            }else{
        ...
        if(((s.length() +1) == tdash.length()) && deleted <= 1) return true;


        if(s.length() == tdash.length()) {
            int wrongg = 0;
            for(int ii = 0; ii < s.length(); ii++) {
                if(s.charAt(ii) != tdash.charAt(ii)) wrongg++;
            }
            if(wrongg == 1) return true;
        }
    ...
    private static String[] readarr(FastScanner sc, int n) {
