...
//        out.println(answer);
        int count = countSquareNumbers(s);
        System.out.println("Number of square numbers: " + count);
    ...
    // Precompute perfect squares and store the results in the array.



    public static int countSquareNumbers(String S) {
        int N = S.length();
    ...
    public static int countSquareNumbersHelper(Map<Integer, Integer> frequency, int number, int remaining) {
        if (remaining == 0) {
            return isPerfectSquare(number) ? 1 : 0;
        }
        ...
        for (int i = 1; i <= 9; i++) {
            if (frequency.containsKey(i) && frequency.get(i) > 0) {
                ...
                count += countSquareNumbersHelper(frequency, number * 10 + i, remaining - 1);
                frequency.put(i, frequency.get(i) + 1);
            }
        }
