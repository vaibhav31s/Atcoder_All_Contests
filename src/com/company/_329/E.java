...
//            int[] arr = readarr(sc, n);
            ...
            for(char ch : s.toCharArray()) first.add(ch);
            for(char ch : tt.toCharArray()) second.add(ch);

            if(tt.charAt(0) == s.substring(0, m).charAt(0) && tt.charAt(m-1) == s.substring(n - m, n).charAt(m-1) && first.equals(second)){
                yes();
            }else no();
        }
    }
