...
//            int[] arr = readarr(sc, n);
            ...
            for(char ch : arr) map1.put(ch,map1.getOrDefault(ch,0)+1);
            for(char ch : sec) map2.put(ch,map2.getOrDefault(ch,0)+1);
...
//            out.println(map2);
            ...
            for(char ch = 'a'; ch <= 'z';      ch++){
                if(map1.containsKey(ch) && map2.containsKey(ch)){
                    int a =map1.get(ch) - map2.get(ch);
                    if(a == 0)continue ;
                    if(a <= 0)
                    map4.put(ch,Math.abs(map1.get(ch) - map2.get(ch)));
                    else map3.put(ch,Math.abs(map1.get(ch) - map2.get(ch)));
                }else{
                    if(map1.containsKey(ch))
                        map3.put(ch, map1.get(ch));

                    if(map2.containsKey(ch))   map4.put(ch, map2.get(ch));


                }
            }
            int a = 0;
            for(char ch : "atcoder".toCharArray()){
                a += map3.getOrDefault(ch,0);
                map3.remove(ch);
            }

            int b = 0;
            for(char ch : "atcoder".toCharArray()){
                b += map4.getOrDefault(ch,0);
                map4.remove(ch);
            }

            if(b <= map1.get('@') && a <= map2.get('@')){
...
//            out.println(map4);
    ...
    private static boolean isVal(char c) {
