...
//            int[] arr = readarr(sc, n);
            PriorityQueue<KeyValue<Long,Long>> pq = new PriorityQueue<>(
                    (a,b)->Long.compare(b.value - a.key, a.value - a.key)
            ...
            for(int i = 0;i < n; i++){
                ...
                long b = sc.nextInt();
                sum += b;
                pq.add(new KeyValue<>(a,b));
            }

            while(sum > k && !pq.isEmpty()){
                KeyValue<Long,Long> kv = pq.poll();
                sum -= kv.value;
                answer = 1 + kv.key;
            }
            assert pq.peek() != null;
